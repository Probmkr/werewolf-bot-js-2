...
- **前提**
  ...
  - **人狼専用チャット**: 人狼陣営（=人狼同士）だけが相談できる「専用チャット」を用意する。
    - **採用案（Bot DM 経由の中継チャット）**: 人狼プレイヤーが Bot に DM で送ったメッセージを、同村の他の人狼プレイヤーへ Bot が DM 転送する。
    - **理由**: ギルド内チャンネルの権限設計は、管理者など一部権限では閲覧制限が通用しない可能性があり、秘匿性を担保しづらい。
    - **補足（将来拡張）**: ギルド側の権限ポリシーが村運用で担保できる環境では、専用チャンネル方式をオプションとして追加できる余地は残す。
    - **補足**: 「1村=1チャンネル」の前提と同様に、専用チャットも「1村=1つ」を維持して混線を防ぐ。
- **フェーズ一覧**
  1. **ロビー/募集フェーズ**
    - コマンドで村を作成し、参加者を募集。
